...
def test_delete_test_case(client, api_test_case, auth_headers):
    response = client.delete(f"/api/test-cases/{api_test_case.id}", headers=auth_headers)
    assert response.status_code == 200
    assert response.json()["ok"] is True


def test_delete_test_case_not_found(client, auth_headers):
    response = client.delete("/api/test-cases/nonexistent", headers=auth_headers)
    assert response.status_code == 404
