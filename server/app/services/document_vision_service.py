...
async def describe_requirement_images(files: list[FileAsset], user_id: str) -> str:
    ...
    system_prompt = (
        "你是需求文档图片识别助手。你的任务是尽可能详细、客观地识别截图、原型图、流程图、表格和扫描图片中的需求信息，"
        "为后续需求解析提供可直接使用的事实证据。"
    )
    prompt = (
        "请逐张识别这些需求文档图片。\n"
        "输出纯文本，不要 Markdown 表格，不要编造；看得清的内容要尽量完整提取，看不清的内容必须放入“不确定内容”。\n"
        "每张图片按以下格式输出：\n"
        "图片来源：对应来源\n"
        "图片类型：页面截图/流程图/表格/原型/接口说明/其他\n"
        "页面或模块：识别页面标题、菜单路径、业务模块或流程名称；无法判断写“无法判断”。\n"
        "可见文字：逐项列出图片中清晰可读的标题、说明、提示语、表格列名、状态值和错误文案。\n"
        "字段与控件：列出输入框、下拉框、复选框、按钮、链接、表格列、必填标记、默认值、占位提示和校验提示。\n"
        "流程与规则：列出流程节点、箭头方向、判断条件、状态流转、权限范围、数据范围、异常处理和预期结果。\n"
        "可形成需求的事实：用短句列出可直接写入需求 rule 的确定信息。\n"
        "不确定内容：只列出看不清、被遮挡、箭头关系不明、含义无法判断或图片本身没有说明的信息。\n\n"
