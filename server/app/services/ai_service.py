...
class AIService:
    ...
    async def parse_requirements(self, file_content: str, user_id: str = "") -> list[dict]:
        ...
        user_prompt = (
            ...
            "2. 如果文档包含【图片识别结果】，必须把图片中的页面、字段、按钮、流程、权限、状态、异常提示与正文、表格合并分析。\n"
            "3. 先用正文、表格、图片识别结果互相补足需求规则；能明确推出的内容写入 rule，不要放入 question。\n"
            "4. question 只记录真实缺口：权限边界、数据范围、状态条件、业务对象指代、异常提示、字段校验等在所有输入中都无法确认时，才逐条提问。\n"
            "5. 不要生成泛化待确认问题，例如“是否按实际系统为准”“是否需要确认权限范围”“字段规则待确认”。必须指出具体缺少哪项信息。\n"
            "6. 如果出现“创建人A/用户A/部门A/非权限范围数据/指定状态的数据/已上传文件/审批人/跨部门数据”等表述，但正文、表格和图片均未定义对象含义、来源或验证口径，才在 question 中逐条提问。\n"
            "7. 辅助文档中的账号、部门、样本数据等只作为【辅助文档信息】写入 question，不要当作待确认问题。\n\n"
            f"文档内容：\n\n{doc_text}"
        )
